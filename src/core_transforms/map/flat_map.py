"""
FlatMap | Apply a function that returns an iterable and flatten results into a single
PCollection.

Unlike Map (one-to-one), FlatMap allows each element to produce zero or more output elements.
Use it as a concise alternative to ParDo when the expansion logic fits in a single
generator function.
